...
SELECT
	...
	CONCAT(
		...
		'<a onclick="Etat(',type_mine.ID,',',mine.etat,')"> ',mine.etat,'%</a></td>\n\t\t<td>',
		'<a onclick="Nombre(',type_mine.ID,',',mine.nombre,')"> ',mine.nombre,'</a></td>\n\t\t<td>',
		REPLACE(REPLACE(FORMAT(mine.prod_max*mine.etat/100,1),',',' '),'.',','),' ',unites.nom,'/h</td>\n\t\t<td>',
		'<a onclick="Production(',type_mine.ID,',',mine.prod_max,')"> ',REPLACE(REPLACE(FORMAT(mine.prod_max,0),',',' '),'.',','),' ',unites.nom,'/h</a></td>\n'
	) AS code,
