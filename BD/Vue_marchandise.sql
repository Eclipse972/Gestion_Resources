...
--
-- Structure de la vue `Vue_marchandise`
--

CREATE ALGORITHM=UNDEFINED DEFINER=`gestion.resources`@`172.20.%` SQL SECURITY DEFINER VIEW `Vue_marchandise` AS select `marchandise`.`ID` AS `ID`,concat(_utf8'		<td><a href="?id=',`marchandise`.`ID`,_utf8'#selection"><img src="Vue/images/',`marchandise`.`image`,_utf8'.png" alt ="',`marchandise`.`nom`,_utf8'">',ucase(left(`marchandise`.`nom`,1)),substr(`marchandise`.`nom`,2,length(`marchandise`.`nom`)),_utf8'</a></td>\n',_utf8'		<td>',replace(format(`marchandise`.`cours_ki`,0),_utf8',',_utf8' '),_utf8'&euro;</td>\n',_utf8'		<td>',replace(format(`marchandise`.`cours_max`,0),_utf8',',_utf8' '),_utf8'&euro;</td>\n') AS `code`,`marchandise`.`nom` AS `nom_ligne` from `marchandise` where (`marchandise`.`nature_ID` between 1 and 2);

--
-- VIEW  `Vue_marchandise`
-- Données: aucune
--
